...
import { canAccessMobileManagementReports, getMobileManagementReportDetail } from '@/lib/mobile-management-reports';
...
export const GET = withAuth<{ id: string }>(async ({ params, user }) => {
  ...
  const payload = await getMobileManagementReportDetail(prisma, user, params.id);
