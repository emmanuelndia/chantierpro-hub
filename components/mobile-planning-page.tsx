...
export function MobilePlanningPage({ user }: MobilePlanningPageProps) {
  ...
  const [mainResourceSearch, setMainResourceSearch] = useState('');
  ...
  const filteredAssignableResources = useMemo(
    () => filterAssignableResources(data?.unassignedSupervisors ?? [], mainResourceSearch),
    [data?.unassignedSupervisors, mainResourceSearch],
  );
  const visibleAssignmentGroups = showAllAssignments ? assignmentGroups : assignmentGroups.slice(0, 3);
  const hiddenAssignmentGroupCount = Math.max(0, assignmentGroups.length - visibleAssignmentGroups.length);
      ...
      {data ? (
        <>
          ...
          <section className="grid grid-cols-4 gap-2">
            <StatTile label="Taches" value={data.assignments.length} />
            <StatTile label="Libres" value={data.unassignedSupervisors.length} />
            <StatTile label="Sites" value={data.availableSites.length} />
            <StatTile label="Date" value={formatShortDate(selectedDateObject)} />
          </section>

          <PlanningActionPanel
            canCreate={canOpenCreateForm}
            freeResourceCount={data.unassignedSupervisors.length}
            siteCount={data.availableSites.length}
            taskCount={data.assignments.length}
            onCreate={() => openCreateForm()}
          />

          <AssignableResourcesPanel
            resources={filteredAssignableResources}
            totalCount={data.unassignedSupervisors.length}
            search={mainResourceSearch}
            showAll={showAllResources}
            onSearchChange={setMainResourceSearch}
            onShowAllChange={setShowAllResources}
            onAssign={(supervisorId) => openCreateForm(supervisorId)}
            emptyDescription={getResourceEmptyDescription(data.availableSites.length)}
          />

          <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <SectionTitle label="Taches assignees" count={data.assignments.length} />
              {assignmentGroups.length > 3 ? (
                <button
                  type="button"
                  onClick={() => setShowAllAssignments((current) => !current)}
                  className="min-h-10 rounded-full border border-slate-200 bg-white px-3 text-xs font-black uppercase tracking-[0.12em] text-slate-700"
                >
                  {showAllAssignments ? 'Reduire' : `Voir ${hiddenAssignmentGroupCount} +`}
                </button>
              ) : null}
            </div>
            ...
            {data.assignments.length > 0 ? (
              <div className="space-y-3">
                {visibleAssignmentGroups.map((group) => (
                  <AssignmentGroupCard
                    ...
                    onDelete={(id) => {
                      if (window.confirm('Retirer cette tache du planning ?')) {
              ...
              <EmptyState title="Aucune tache assignee" description="Ajoutez une tache pour preparer la journee terrain." />
            )}
          </section>
        </>
      ) : null}
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-panel">
      <div className="bg-slate-950 p-4 text-white">
        <p className="text-[11px] font-black uppercase tracking-[0.18em] text-sky-200">Action planning</p>
        <div className="mt-2 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h2 className="text-xl font-black leading-tight">Nouvelle tache</h2>
            <p className="mt-1 text-xs font-semibold text-slate-300">
              {freeResourceCount} ressource(s) libre(s), {siteCount} chantier(s), {taskCount} tache(s)
            </p>
          </div>
          <button
            type="button"
            onClick={onCreate}
            disabled={!canCreate}
            className="min-h-12 shrink-0 rounded-xl bg-white px-4 text-sm font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
          >
            Ajouter
          </button>
        </div>
      ...
      {!canCreate ? (
        <p className="px-4 py-3 text-xs font-semibold leading-5 text-slate-500">
          Ajout indisponible tant qu&apos;il manque une ressource libre ou un chantier/projet actif.
        </p>
      ) : null}
    </section>
  );
...
function AssignableResourcesPanel({
  resources,
  totalCount,
  search,
  showAll,
  ...
  const hiddenCount = Math.max(0, resources.length - visibleResources.length);

  return (
    <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-panel">
      <div className="flex items-center justify-between gap-3">
        <SectionTitle label="Ressources libres" count={totalCount} />
        {resources.length > 6 ? (
          <button
            type="button"
            onClick={() => onShowAllChange(!showAll)}
            className="min-h-10 rounded-full border border-slate-200 bg-slate-50 px-3 text-xs font-black uppercase tracking-[0.12em] text-slate-700"
          >
            {showAll ? 'Reduire' : '+ ' + hiddenCount}
          </button>
        ) : null}
      </div>
      ...
      {totalCount > 4 ? (
        <input
          ...
          className="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-800 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
          placeholder="Rechercher une ressource..."
        />
      ) : null}

      {totalCount === 0 ? (
        <EmptyState title="Aucune ressource terrain active disponible" description={emptyDescription} />
      ) : resources.length === 0 ? (
        ...
        <div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-1">
          {visibleResources.map((supervisor) => (
            <ResourceMiniCard key={supervisor.id} supervisor={supervisor} onAssign={() => onAssign(supervisor.id)} />
          ))}
        </div>
      )}
    </section>
...
function ResourceMiniCard({ supervisor, onAssign }: Readonly<{ supervisor: UnassignedSupervisor; onAssign: () => void }>) {
  return (
    <article className="min-w-[168px] max-w-[168px] rounded-2xl border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-start gap-2">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-xs font-black text-slate-800 shadow-sm">
      ...
      <p className="mt-2 line-clamp-1 text-[11px] font-black uppercase tracking-[0.12em] text-sky-700">
        {supervisor.availabilityLabel}
      </p>
      <button
        type="button"
        onClick={onAssign}
        className="mt-3 min-h-10 w-full rounded-xl bg-slate-950 px-3 text-xs font-black uppercase tracking-[0.1em] text-white"
      >
        Assigner
      </button>
    </article>
...
function StatTile({ label, value }: Readonly<{ label: string; value: string | number }>) {
  return (
    <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3 text-center shadow-sm">
      ...
      <div className="mt-1 truncate text-[10px] font-black uppercase tracking-[0.1em] text-slate-500">{label}</div>
    </div>
  );
}
