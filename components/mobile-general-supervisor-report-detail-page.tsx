...
import type { GeneralSupervisorReportDetailResponse } from '@/types/mobile-general-supervisor-reports';

type MobileGeneralSupervisorReportDetailPageProps = Readonly<{
  reportId: string;
}>;

export function MobileGeneralSupervisorReportDetailPage({ reportId }: MobileGeneralSupervisorReportDetailPageProps) {
  const reportQuery = useQuery({
    queryKey: ['mobile-general-supervisor-report-detail', reportId],
    queryFn: async () => {
      const response = await authFetch(`/api/mobile/general-supervisor/reports/detail/${reportId}`);
      if (!response.ok) {
        throw new Error(`Report detail request failed with status ${response.status}`);
      }
      return (await response.json()) as GeneralSupervisorReportDetailResponse;
    },
    staleTime: 30_000,
  });
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-panel">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Rapport terrain</p>
            <h1 className="mt-1 text-xl font-black text-slate-950">{report.siteName}</h1>
            <p className="mt-1 text-sm font-semibold text-slate-600">{report.authorName}</p>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <Metric label="Heure" value={report.session.time.slice(0, 5)} />
