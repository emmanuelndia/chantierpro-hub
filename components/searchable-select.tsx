...
export function SearchableMultiSelect({
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      <div
        className={`min-h-[48px] rounded-2xl border border-slate-200 bg-white px-3 py-2 transition focus-within:border-orange-500 ${
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          {selectedOptions.map((option) => (
            <span
              className="inline-flex max-w-full items-center gap-2 rounded-full bg-slate-950 px-3 py-1.5 text-xs font-semibold text-white"
              key={option.value}
            >
          ...
          <input
            className="min-w-[150px] flex-1 border-0 bg-transparent px-1 py-1.5 text-sm text-slate-900 outline-none placeholder:text-slate-400 disabled:cursor-not-allowed"
            disabled={disabled}
            onChange={(event) => {
              setQuery(event.target.value);
              setIsOpen(true);
            ...
            placeholder={selectedOptions.length > 0 ? 'Ajouter...' : placeholder}
            type="search"
            value={query}
          />
          <ChevronDown className="h-4 w-4 text-slate-400" />
      ...
      {selectedOptions.length > 0 ? (
        <p className="mt-1 text-xs font-semibold text-slate-500">{selectedOptions.length} ressource(s) selectionnee(s)</p>
      ) : null}
